...
// ========================================================================================================
// === F# / Console Application fake build ======================================================== 1.0.0 =
// --------------------------------------------------------------------------------------------------------
// Options:
...
let project = "Translation Console"
let summary = "Simple console application to extract/map translates in files."
...
Target.create "Lint" <| skipOn "no-lint" (fun _ ->
    ...
    !! "src/*.fsproj"   // todo - now it fails in tests for msbuild - dont know why, it should be `*/*.fsproj`
    -- "example/*.*proj"
    |> Seq.map (fun fsproj ->
        match toolsDir with
        | Global ->
...
Target.create "Release" (fun _ ->
    ...
    |> List.iter (fun runtimeId ->
        sprintf "publish -c Release /p:PublishSingleFile=true -o ./dist/%s --self-contained -r %s translation-console.fsproj" runtimeId runtimeId
